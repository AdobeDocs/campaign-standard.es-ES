...
| Variable | Valor |
| :-: | :-: |
| deliveryId | `deliveryId` a partir de datos entrantes (similar al seguimiento push donde se usa `_dld`) |
| broadlogId | `broadlogId` a partir de datos entrantes (similar al seguimiento push donde se usa `_mld`) |
...
Para el rastreo de clics, debe enviar el valor &quot;2&quot; para que se lleve a cabo una acción al llamar a las funciones `collectMessageInfo()` o `trackAction()`.
...
* El usuario ve la notificación, pero la borra.
  ...
  Para rastrear clics en caso de descarte, agregue el receptor de difusión `NotificationDismissalHandler` al archivo AndroidManifest del módulo de la aplicación.
...
Este código se basa en la implementación del seguimiento de impresiones de clics. Con la intención definida, ahora debe devolver la información de seguimiento a Adobe Campaign. En este caso, la vista de Android ([!DNL Activity]) que activó la notificación se abrirá o se colocará en primer plano como resultado del clic del usuario. El objeto Intent de [!DNL Activity] contiene los datos de notificación que se pueden usar para realizar el seguimiento de las aperturas.

MainActivity.java (amplía [!DNL Activity])
