...
Se puede encontrar más información sobre los campos de entidad de perfil [aquí](../../developing/using/datamodel-profile.md).
...
* **Unidireccional (Campaign a Microsoft Dynamics 365)**: el Campaign Standard es la fuente fiable para las exclusiones. Los atributos de exclusión se sincronizarán en una dirección desde Campaign Standard a Dynamics 365
...
Si seleccionó la **Bidireccional** o **Unidireccional (Campaign a Microsoft Dynamics 365)** Configuración de exclusión, los datos de exclusión de Campaign se exportarán periódicamente a través del flujo de trabajo al área de almacenamiento SFTP de Campaign (consulte &quot;Uso de SFTP de Campaign a continuación&quot;). En caso de que los flujos de trabajo de exclusión de Campaign dejen de ejecutarse, deberá reiniciarse manualmente lo antes posible para reducir la posibilidad de que se pierdan sincronizaciones de exclusión.

>[!IMPORTANT]
>
>Si necesita el **Bidireccional** o **Unidireccional (Campaign a Microsoft Dynamics 365)** configuración de exclusión, deberá enviar la solicitud a su contacto técnico de Adobe para que se configuren los flujos de trabajo de exclusión en la instancia de Campaign
...
El [Integración de Microsoft Dynamics 365-Adobe Campaign Standard](../../integrating/using/d365-acs-get-started.md) admite entidades personalizadas, lo que permite sincronizar las entidades personalizadas de Dynamics 365 con los recursos personalizados correspondientes de Campaign.
...
* Si el elemento principal y el secundario están vinculados en el lado de la campaña mediante **1 enlace simple de cardinalidad** , el registro secundario permanecerá oculto e inaccesible (a través de la interfaz de usuario o la API) hasta que el registro principal llegue a Campaign.

* (Suponiendo **1 enlace simple de cardinalidad** en Campaign) Si el registro secundario se actualiza o elimina en Dynamics 365 y ese cambio se escribe en Campaign antes de que el registro principal aparezca en Campaign (no es probable, pero es una posibilidad remota), esa actualización o eliminación no se procesará en Campaign y se generará un error. En caso de actualización, el registro en cuestión deberá actualizarse de nuevo en Dynamics 365 para sincronizar el registro actualizado. En caso de eliminación, el registro en cuestión deberá tratarse por separado en el lado de la campaña, ya que ya no hay un registro en Dynamics 365 para eliminar o actualizar.

* Si se encuentra con una situación en la que cree que tiene registros secundarios ocultos y no hay forma de acceder a ellos, puede cambiar temporalmente el tipo de vínculo de cardinalidad a **0 o 1 enlace simple de cardinalidad** para acceder a esos registros.
...
* Deberá obtener la licencia del paquete de Campaign adecuado para admitir el volumen de llamadas al motor generado por la integración. Si se supera el volumen de llamadas al motor con licencia, podría causar una degradación en el rendimiento de la campaña.
  ...
  Consulte la información del paquete de Adobe Campaign Standard aquí: [https://helpx.adobe.com/legal/product-descriptions/campaign-standard.html](https://helpx.adobe.com/es/legal/product-descriptions/campaign-standard.html)
...
* La integración admite hasta 5 columnas vinculadas por recurso personalizado. La vinculación de varias columnas entre recursos personalizados puede tener un impacto significativo en el rendimiento. **0 o 1 enlace simple de cardinalidad** se prefiere en lugar de **1 enlace simple de cardinalidad**.
...
* La integración hace lo siguiente **no** admitir la remodelación de datos entre los modelos de datos de Dynamics 365 y Campaign Standard. Se espera que la integración sincronice una tabla de Dynamics 365 con una tabla de Campaign.
